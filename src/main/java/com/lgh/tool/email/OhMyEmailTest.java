...
import static com.lgh.tool.email.OhMyEmail.SMTP_163;

/**
 * @ClassName OhMyEmailTest
 * @Author lgh
 * @Date 2020/11/3 9:46
 **/
public class OhMyEmailTest {
    ...
    // 该邮箱修改为你需要测试的邮箱地址
    private static final String TO_EMAIL = "liu.guang.hu@qq.com";

    @Before
    public void before() {
        // 配置，一次即可
        OhMyEmail.config(SMTP_163(true), "lghwuwangchuxin@163.com", "lgh2019");
        // 如果是企业邮箱则使用下面配置
    ...
    public void testSendAttach() throws SendMailException {
        OhMyEmail.subject("这是一封测试附件邮件")
                ...
                .to(TO_EMAIL)
                .html("<h1 font=red>信件内容</h1>")
                .attach(new File("C:/Users/Administrator/Desktop/jjj.csv"), "cs.csv")
                .send();
        Assert.assertTrue(true);
    }
